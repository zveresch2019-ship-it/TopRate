...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // Получаем shared userId для общих данных
    const sharedUserId = await getSharedUserId();
    
    // Админы и обычные пользователи видят общих игроков
...
router.post('/',
  ...
  async (req, res) => {
    try {
      ...
      // Админы создают игроков в общую базу, обычные пользователи - в свою
      const userIsAdmin = await isAdmin(req.userId);
      const targetUserId = userIsAdmin ? await getSharedUserId() : req.userId;
      
      // Get current season for this sport (используем сезон shared пользователя для админов)
      const currentSeason = await Season.findOne({ 
        userId: targetUserId,
        sportType: currentSportType,
        isActive: true 
      ...
      const existingPlayer = await Player.findOne({
        $or: [
          { userId: req.userId },
          { userId: sharedUserId } // Проверяем и в общих данных
        ],
        name: name.trim(),
        currentSeason: seasonNumber,
        sportType: currentSportType,
      ...
      const player = new Player({
        userId: targetUserId, // Используем shared userId для админов
        name: name.trim(),
        currentSeason: seasonNumber,
