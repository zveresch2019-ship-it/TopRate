...
const HomeScreen: React.FC<HomeScreenProps> = ({navigation}) => {
  ...
  const createBeautifulTextTable = () => {
    ...
    if (currentSeason) {
      ...
      text += `📊 Всего игроков: ${totalPlayers}\n`;
      text += `⚽ Всего матчей: ${totalMatches}\n`;
      text += `📈 Средний рейтинг: ${averageRating}\n\n`;
    }
    
    text += '🏆 ТОП ИГРОКОВ:\n';
    text += '═══════════════════════════════════════\n';
    text += '№ | Имя        | Было | Изм | Стало | Игры | Всего\n';
    text += '──┼────────────┼──────┼─────┼───────┼──────┼──────\n';
    
    ranking.slice(0, 10).forEach((player, index) => {
      const ratingBeforeLastGame = player.rating - player.lastRatingChange;
      const totalRatingGain = player.totalRatingGain;
      
      const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : '';
      
      text += `${medal || (index + 1)} | ${player.name.length > 10 ? player.name.substring(0, 10) : player.name} | ${ratingBeforeLastGame} | ${player.lastRatingChange > 0 ? '+' : ''}${player.lastRatingChange} | ${player.rating} | ${player.matchesPlayed} | ${totalRatingGain > 0 ? '+' : ''}${totalRatingGain}\n`;
    });
      ...
      {ranking.length > 0 && (
        <View style={styles.topPlayersContainer}>
          ...
          {ranking.slice(0, 3).map((player, index) => {
            const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : '';
            return (
              <View key={`${player.id}-${index}`} style={styles.topPlayerCard}>
                ...
                <View style={styles.ratingChanges}>
                  <Text style={styles.ratingChange}>
                    {player.lastRatingChange > 0 ? '+' : ''}{player.lastRatingChange}
                  </Text>
                  <Text style={styles.seasonChange}>
                    {player.totalRatingGain > 0 ? '+' : ''}{player.totalRatingGain}
                  </Text>
                </View>
              </View>
            );
