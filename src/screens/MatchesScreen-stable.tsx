...
const MatchesScreen: React.FC = () => {
  ...
  const handleDeleteMatch = async (matchId: string) => {
    ...
    if (isLastMatchCancelled) {
      Alert.alert(
        'Отмена уже произведена',
        'Отмена последнего матча уже была произведена. Добавьте новый матч, чтобы снова получить возможность отмены.',
    ...
    Alert.alert(
      'Отменить последний матч',
      'Вы уверены, что хотите отменить последний матч? Рейтинги игроков вернутся к значениям до этого матча.',
      [
        {text: 'Отмена', style: 'cancel'},
        {
          ...
          onPress: async () => {
            ...
            if (success) {
              ...
              Alert.alert('Успех', 'Матч отменен! Рейтинги восстановлены.');
            } else {
              Alert.alert('Ошибка', 'Не удалось отменить матч.');
            }
          },
        },
      ]
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          <Text style={styles.toggleParamsText}>
            {showCalculationParams ? '📊 Скрыть параметры' : '📊 Показать параметры'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {showCalculationParams && (
        <View style={styles.calculationParams}>
          <Text style={styles.paramsTitle}>📊 Параметры расчета:</Text>
          <Text style={styles.debugInfo}>
            Отладка: Команда A: {homeTeam.playerIds.length} игроков, Команда B: {awayTeam.playerIds.length} игроков
          </Text>
          {(() => {
            ...
            return params ? (
              <View style={styles.paramsList}>
                <View style={styles.paramRow}>
                  <Text style={styles.paramLabel}>ES (Ожидаемая разница):</Text>
                  <Text style={styles.paramValue}>{params.ES.toFixed(2)}</Text>
                </View>
                <View style={styles.paramRow}>
                  <Text style={styles.paramLabel}>RGD (Реальная разница):</Text>
                  <Text style={styles.paramValue}>{params.RGD.toFixed(2)}</Text>
                </View>
                <View style={styles.paramRow}>
                  <Text style={styles.paramLabel}>GV (Цена гола):</Text>
                  <Text style={styles.paramValue}>{params.GV.toFixed(2)}</Text>
                </View>
                <View style={styles.paramRow}>
                  <Text style={styles.paramLabel}>TV (Итоговая сумма):</Text>
                  <Text style={styles.paramValue}>{Math.round(params.TV)}</Text>
                </View>
              </View>
            ) : (
              <Text style={styles.noParamsText}>
                Добавьте игроков в обе команды для расчета параметров
              </Text>
            );
          })()}
        </View>
      ...
      <View style={styles.swipeLayout}>
        ...
        <View style={styles.teamsHeader}>
          <View style={styles.teamHeader}>
            <Text style={styles.teamTitle}>Команда A</Text>
            <Text style={styles.teamStats}>
              {homeTeam.playerIds.length} игроков • Рейтинг: {homeTeam.totalRating}
            </Text>
          </View>
          <View style={styles.teamHeader}>
            <Text style={styles.teamTitle}>Команда B</Text>
            <Text style={styles.teamStats}>
              {awayTeam.playerIds.length} игроков • Рейтинг: {awayTeam.totalRating}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.playersListContainer}>
          <Text style={styles.playersListTitle}>
            Доступные игроки ({players.filter(p => !homeTeam.playerIds.includes(p.id) && !awayTeam.playerIds.includes(p.id)).length})
          </Text>
        ...
        <View style={styles.teamsContainer}>
          <View style={styles.teamPlayersContainer}>
            <Text style={styles.teamPlayersTitle}>Команда A</Text>
          ...
          <View style={styles.teamPlayersContainer}>
            <Text style={styles.teamPlayersTitle}>Команда B</Text>
            <ScrollView style={styles.teamPlayersList} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.actionButtons}>
        <TouchableOpacity style={styles.saveButton} onPress={handleSaveMatch}>
          <Text style={styles.saveButtonText}>💾 Сохранить матч</Text>
        </TouchableOpacity>
      </View>
